...
extension LogEntryRepository {
    ...
    func logMeal(
        ...
        try PersistenceReporter.persist(in: modelContext.container, operation: operation) { isolatedContext in
            guard let isolatedMeal = try MealRepository(modelContext: isolatedContext).fetchMeal(id: mealID) else {
                throw NSError(
            ...
            let summary = try MealRepository(modelContext: isolatedContext).nutritionSummary(for: isolatedMeal)
    ...
    private func mealComponentSnapshots(for logEntryID: UUID, in context: ModelContext) throws -> [LoggedMealComponentSnapshot] {
