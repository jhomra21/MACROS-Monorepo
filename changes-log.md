...
- Added a Full Unlock-gated native Dashboard macro long-press menu with only a per-macro `Change Color` action, using `UIContextMenuInteraction` so each macro owns its hold target without a custom preview overlay.
- A simplify review moved the shared UIKit color picker/save helper out of `SettingsScreen.swift`, removed duplicate Dashboard/Settings picker persistence logic, tied the macro legend width math to `MacroMetric.allCases`, and kept one tap path per rendered macro branch.
- A defensive-code review found no high-confidence redundant guards, duplicated validation, or impossible-state branches in the Dashboard macro long-press follow-up.
- The Dashboard macro long-press follow-up passed whitespace diff validation, formatter validation, iOS simulator build, simplify review, defensive-code review, and final diff review.
...
- Updated `DailyMacroWidget.swift` so `.systemMedium` computes its ring and metric-column widths from widget geometry, keeping the ring fully visible while giving each macro a dedicated column.
